...
function PhrasalVerbQuiz({
  ...
  return (
    <QuizEngine<PhrasalVerbQuizQuestion, PhrasalVerbQuizAnswer, PhrasalVerbMistake>
      createQuestions={() => buildPhrasalVerbQuiz(category)}
      renderEmpty={() => (
      ...
      onComplete={recordPhrasalVerbQuizResult}
      buildAnswer={({ question, isCorrect }) => ({
        phrasalVerbId: question.phrasalVerb.id,
        correct: isCorrect,
      })}
      buildMistake={({ question, option }) => ({
        phrasalVerb: question.phrasalVerb,
        selectedMeaning: option,
      })}
      autoAdvanceDelay={1100}
      onBack={onBack}
      backLabel="← К фразовым глаголам"
      questionLabel="Что это значит?"
      getQuestionTitle={(question) => question.phrasalVerb.phrase}
      getSpeechText={(question) => question.phrasalVerb.phrase}
    />
  )
}
