  ...
  if (pool.length === 0 || questions.length === 0) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center px-6 text-center">
        <p className="text-slate-500">Не удалось собрать квиз по идиомам.</p>
        <button
          type="button"
          onClick={onBack}
              ...
              {mistakes.map(({ idiom, selectedMeaning }) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h3 className="text-lg font-semibold text-slate-950">{idiom.phrase}</h3>
                  ...
                  <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm leading-relaxed">
                    ...
                    <p className="mt-3 text-slate-700">{idiom.example}</p>
                    <p className="mt-1 text-slate-500">{idiom.exampleTranslation}</p>
                  </div>
                </li>
              ))}
            </ul>
