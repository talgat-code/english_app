...
function Review() {
  ...
  if (finished) {
    return (
      <div className="flex min-h-screen w-full flex-col justify-center px-5 py-8">
        ...
        {hardWords.length > 0 && (
          <button
            type="button"
            onClick={restart}
            className="mt-6 min-h-12 w-full rounded-2xl bg-indigo-600 px-6 py-3 text-base font-semibold text-white shadow-sm transition-colors hover:bg-indigo-700 active:bg-indigo-800"
          >
            Повторить ещё раз
          </button>
        )}
  ...
  if (questions.length === 0) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center px-6 text-center">
        ...
        <p className="mt-2 text-slate-500">Пройди несколько квизов</p>
      </div>
    )
  }
    ...
    <div className="flex min-h-screen w-full flex-col px-5 py-6">
      <header className="mb-8">
        <div className="flex items-center justify-between">
          <h1 className="text-lg font-bold text-slate-900">🔄 Повторение</h1>
